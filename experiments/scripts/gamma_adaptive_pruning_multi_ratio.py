...
实验流程:
1. 计算参数重要性得分（只计算一次）
2. 为每层拟合 Gamma 分布 (k_ℓ, θ_ℓ)（只拟合一次）
3. 缓存评估数据批次（确保所有剪枝比例使用相同数据）
4. 对每个剪枝比例:
   - 求解全局阈值 τ*
   - 计算各层自适应剪枝率
   - 执行剪枝并评估损失
...
def main():
    ...
    parser = argparse.ArgumentParser(description='基于 Gamma 分布的自适应剪枝实验 - 多剪枝比例版本')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='results/gamma_adaptive_pruning_multi',
                        help='输出目录')
    ...
    print("=" * 80)
    print("基于 Gamma 分布的自适应剪枝实验 - 多剪枝比例版本")
    print("=" * 80)
