"""Legacy single-checkpoint method-comparison diagnostic.

This script is not the authoritative paired paper-evidence runner.
固定剪枝比例下的方法对比诊断。
...
def main():
    parser = argparse.ArgumentParser(description="Legacy single-checkpoint method diagnostic")
    parser.add_argument("--model", type=str, required=True, help="模型名称")
    parser.add_argument("--dataset", type=str, required=True, help="数据集名称")
    parser.add_argument("--checkpoint", type=str, default=None, help="微调检查点路径")
    parser.add_argument(
        "--reference_checkpoint", type=str, default=None, help="参考检查点路径（ExCP 残差剪枝用）"
    )
    parser.add_argument("--prune_ratios", type=str, default="0.1,0.2,0.3,0.4")
    parser.add_argument(
        "--methods", type=str, default=None, help="指定方法（逗号分隔），默认全部运行"
    )
    parser.add_argument(
        "--alpha",
        type=str,
        default="0.1",
        help="二阶项权重（支持逗号分隔多值扫描，如 0.05,0.1,0.2）",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="给 first-order 方法单独指定梯度 batch 数。" "默认 None 表示与 --hvp_batches 相同。",
    )
    ...
    print("=" * 70)
    print("Legacy single-checkpoint method diagnostic")
    print("=" * 70)
    ...
    if args.methods:
        ...
        selected = normalized
        methods_to_run = [
            m for m in methods_to_run if f"{m['importance']}+{m['allocation']}" in selected
        ]
    ...
    if args.reference_checkpoint:
        ...
        print(
            "  [WARN] 无 --checkpoint 和 --reference_checkpoint，residual-magnitude 残差为 0，已跳过"
        )
            ...
            for alpha_val in alpha_values:
                ...
                if args.scoring_mode == "2d" and "magnitude" in all_scores:
                    ...
                    pruning_scores = combine_scores_2d_with_protection(
                        mag_scores, scores_for_alpha, protection_ratio=args.protection_ratio
                    )
                    allocation_scores_override = scores_for_alpha  # 层间分配用 damage score
                    mode_tag = f"[2D prot={args.protection_ratio}] "
                    print(
                        f"  {mode_tag}组合 magnitude + damage 得分 (protection={args.protection_ratio:.2%})"
                    )
                    ...
                    pruning_scores = apply_magnitude_protection(
                        scores_for_alpha, prunable_w, protection_ratio=args.protection_ratio
                    )
                    mode_tag = f"[protected prot={args.protection_ratio}] "
                    print(
                        f"  {mode_tag}magnitude protection (protection={args.protection_ratio:.2%})"
                    )
    ...
    print_results_table(all_results, ["method", "target_ratio", "actual_ratio", "loss", metric_key])
    print("\n实验完成！")
