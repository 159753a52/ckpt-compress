...
def main():
    ...
    for m in ABLATION_METHODS:
        ...
        for ratio in prune_ratios:
            ...
            result = {
                'label': label, 'importance': imp, 'allocation': alloc,
                'method': method_name, 'target_ratio': ratio, 'actual_ratio': actual,
            }
            result.update(metrics)
            if 'loss' in baseline:
                result['loss_increase_pct'] = (metrics['loss'] - baseline['loss']) / baseline['loss'] * 100
            if 'accuracy' in baseline:
                result['accuracy_drop'] = baseline['accuracy'] - metrics.get('accuracy', 0)
            
            all_results.append(result)
            print(f"  ratio={ratio:.0%} | " + " | ".join(f"{k}={v:.4f}" for k, v in metrics.items()))
    ...
    print_results_table(all_results, ['label', 'importance', 'allocation', 'target_ratio', 
                                       'loss', metric_key])
    print("\n实验完成！")
