...
LOCAL_MODEL_PATH = os.environ.get('PYTHIA_MODEL_PATH', '/root/ckpt-compress/data/models/pythia-410m')
LOCAL_ALPACA_PATH = '/root/ckpt-compress/data/alpaca/alpaca_data.json'
...
def prepare_alpaca_loaders(tokenizer, batch_size, seq_length, num_workers=0):
    ...
    dataset = dataset.map(
        format_and_tokenize, batched=True,
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='checkpoints/pythia_410m_alpaca')
    ...
    # 加载数据
    print("\n加载 Alpaca 52K 数据集...")
    train_loader, val_loader = prepare_alpaca_loaders(
        tokenizer, args.batch_size, args.seq_length)
    ...
    for step in tqdm(range(1, args.total_steps + 1), desc="Training"):
        ...
        loss = train_step(
            model, optimizer, scaler, batch, args.device, args.use_amp)
        scheduler.step()
        running_loss += loss
