...
from dacp.utils.data_loader import get_sst2_loaders, get_mnli_loaders, get_stsb_loaders
...
def train_n_steps(
    ...
    history = {'steps': [], 'train_loss': [], f'eval_{metric_name}': []}
            ...
            if step % save_interval == 0 or step == num_steps:
                if eval_loader is not None:
                    ...
                    history['train_loss'].append(
                        avg_loss if step % log_interval == 0 else 0)
                    ...
                    if eval_metric > best_metric:
                        ...
                        _save_checkpoint(model, optimizer, step, eval_metric,
                                         metric_name, best_path)
...
def _save_checkpoint(model, optimizer, step, eval_metric, metric_name, path):
    """保存检查点。"""
    checkpoint = {
        'step': step,
...
def main():
    parser = argparse.ArgumentParser(
        description='BERT-Large 在 GLUE 任务上微调'
    )
    ...
    parser.add_argument('--matched', action='store_true',
                        help='[MNLI] 使用 matched 验证集（默认 mismatched）')
    ...
    model = BertForSequenceClassification.from_pretrained(
        os.environ.get('BERT_MODEL_PATH', '/root/ckpt-compress/data/models/bert-large-uncased'),
    ...
    if args.dataset == 'sst2':
        ...
        from experiments.lib.data import _get_glue_loaders
        train_loader, val_loader = _get_glue_loaders(
            'bert-large', args.dataset,
            batch_size=args.batch_size,
