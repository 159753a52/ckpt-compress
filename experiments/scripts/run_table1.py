...
from src.ckpt_compress.pruning.per_type_allocation import PerTypeAllocation
...
METHODS = [
    ...
    {'importance': 'first-order',        'allocation': 'gamma-adaptive'},
    ...
    {'importance': 'adam-second-order',  'allocation': 'uniform'},
    {'importance': 'adam-second-order',  'allocation': 'gamma-adaptive'},
]
...
def main():
    ...
    parser.add_argument('--grad_batches_first_order', type=int, default=None,
                        help='给 first-order 方法单独指定梯度 batch 数（故意削弱）。'
                             '设为 1 时 first-order 只用 1 个 batch 估计梯度（高噪声）。'
    ...
    # 尝试从 checkpoint 读取 Adam exp_avg_sq（用于 adam-second-order 方法）
    adam_v = None
    need_adam = any(
        m['importance'] == 'adam-second-order'
        for m in METHODS
    )
    if need_adam and args.checkpoint:
        try:
            ckpt = torch.load(args.checkpoint, map_location='cpu')
            opt_state = None
            if 'optimizer' in ckpt and 'state' in ckpt['optimizer']:
                opt_state = ckpt['optimizer']['state']
            elif 'optimizer_state_dict' in ckpt and 'state' in ckpt['optimizer_state_dict']:
                opt_state = ckpt['optimizer_state_dict']['state']

            if opt_state is not None:
                # 构建 param_name -> exp_avg_sq 的映射
                # optimizer state 用 param_group 中的参数顺序索引
                param_names_ordered = [n for n, _ in model.named_parameters()]
                adam_v = {}
                for idx, name in enumerate(param_names_ordered):
                    if idx in opt_state and 'exp_avg_sq' in opt_state[idx]:
                        adam_v[name] = opt_state[idx]['exp_avg_sq'].detach().cpu()
                if adam_v:
                    print(f"  [adam-second-order] 成功加载 exp_avg_sq，共 {len(adam_v)} 层")
                else:
                    print("  [WARN] checkpoint 中未找到 exp_avg_sq，adam-second-order 方法将被跳过")
                    adam_v = None
            else:
                print("  [WARN] checkpoint 中未找到 optimizer state，adam-second-order 方法将被跳过")
        except Exception as e:
            print(f"  [WARN] 读取 Adam state 失败: {e}，adam-second-order 方法将被跳过")
    elif need_adam and not args.checkpoint:
        print("  [WARN] 未指定 --checkpoint，adam-second-order 方法将被跳过（需要 Adam optimizer state）")
    ...
    # 非 HVP 二阶方法的 importance 列表（magnitude / first-order / residual-magnitude / adam-second-order）
    # adam-second-order 依赖 alpha，单独处理；其余在此批次计算
    basic_imps = [m for m in needed_importances
                  if m not in ('second-order-hvp', 'adam-second-order')]
    need_hvp_second_order = 'second-order-hvp' in needed_importances
    need_adam_second_order = 'adam-second-order' in needed_importances and adam_v is not None

    if need_adam_second_order and adam_v is None:
        print("  [INFO] adam-second-order 被跳过：无法读取 exp_avg_sq")
        methods_to_run = [m for m in methods_to_run if m['importance'] != 'adam-second-order']
        need_adam_second_order = False
    ...
    # 对每个 alpha 值计算 Adam 二阶得分（无 HVP，零噪声）
    adam_alpha_scores = {}  # {alpha_val: {param_name: score_tensor}}
    if need_adam_second_order:
        for alpha_val in alpha_values:
            print(f"\n  [adam-second-order] 计算 alpha={alpha_val} 的得分...")
            model_for_scoring = copy.deepcopy(model).to(args.device)
            adam_so_scores = compute_scores_by_method(
                model_for_scoring,
                cached_train,
                task_type,
                methods=['adam-second-order'],
                alpha=alpha_val,
                hvp_batches=args.hvp_batches,
                hvp_mode=args.hvp_mode,
                chunk_size=args.chunk_size,
            ...
            adam_alpha_scores[alpha_val] = adam_so_scores['adam-second-order']
            del model_for_scoring
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
        ...
        if imp == 'per-type-auto' and per_type_allocator is not None:
            ...
            for alpha_val in alpha_values:
                ...
                results = run_single_method(
                    model_factory, scores_for_alpha, method_with_alpha, prune_ratios,
                ...
                all_results.extend(results)
        elif imp == 'adam-second-order':
            # 对每个 alpha 值分别运行（Adam exp_avg_sq 版二阶，零噪声）
            for alpha_val in alpha_values:
                alpha_suffix = f"(α={alpha_val})" if len(alpha_values) > 1 else ""
                print(f"\n--- {imp} + {alloc} {alpha_suffix} ---")
                scores_for_alpha = adam_alpha_scores[alpha_val]
                method_with_alpha = dict(method)
                if len(alpha_values) > 1:
                    method_with_alpha['_method_name_override'] = f"{imp}+{alloc}(α={alpha_val})"
                results = run_single_method(
