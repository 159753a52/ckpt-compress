...
echo ""
echo -e "${BLUE}剪枝配置:${NC}"
echo "  基于无损剪枝比例，每项减去 0.3（低于 0 则设为 0）"
echo ""
echo "  示例 (Block 0):"
echo "    - attn_qkv:  35% →  5% (35% - 30%)"
echo "    - attn_proj: 85% → 55% (85% - 30%)"
echo "    - mlp_fc:    55% → 25% (55% - 30%)"
echo "    - mlp_proj:  55% → 25% (55% - 30%)"
echo "    - ln_1:      25% →  0% (25% - 30% < 0)"
echo "    - ln_2:      20% →  0% (20% - 30% < 0)"
echo ""
echo -e "${BLUE}统计信息:${NC}"
echo "  原始平均无损剪枝率: 44.4%"
echo "  调整后平均剪枝率: 18.8%"
...
OUTPUT_DIR="results/pruning_test_minus0.3_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
python experiments/scripts/test_pruning_config.py \
    ...
    --config experiments/configs/pruning_config_minus0.3.json \
    --device "$DEVICE" \
