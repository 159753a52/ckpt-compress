...
from dacp.pruning.allocation import GammaAdaptiveAllocation
...
def sweep_hvp_batch(args, model_init, cached_train, cached_eval, task_type, model_family="gpt2"):
    ...
    for hvp_b in hvp_batches_list:
        ...
        row = {
            "hvp_batches": hvp_b,
            "hvp_time_s": round(hvp_time, 3),
            "actual_ratio": actual,
            **metrics,
        }
        results.append(row)
        print(
            f"  hvp_batches={hvp_b:3d} | time={hvp_time:.2f}s | "
            + " | ".join(f"{k}={v:.4f}" for k, v in metrics.items())
        )
...
def sweep_bisection_tol(
    args, model_init, cached_train, cached_eval, task_type, model_family="gpt2"
):
    ...
    for tol in tols:
        ...
        row = {
            "bisection_tol": tol,
            "alloc_time_s": round(alloc_time, 5),
            "actual_ratio": actual,
            **metrics,
        }
        results.append(row)
        print(
            f"  tol={tol:.0e} | alloc_time={alloc_time:.4f}s | "
            + " | ".join(f"{k}={v:.4f}" for k, v in metrics.items())
        )
...
def timing_breakdown(args, model_init, cached_train, cached_eval, task_type, model_family="gpt2"):
...
def main():
    ...
    parser.add_argument("--total_steps", type=int, default=500, help="总训练步数（sweep K 时使用）")
    parser.add_argument("--lr", type=float, default=5e-5)

    # Sweep 参数
    parser.add_argument(
        "--sweep_k", type=str, default="5,10,20,50", help="compression-restore 次数列表"
    )
    parser.add_argument(
        "--sweep_hvp_batches", type=str, default="1,4,8,16", help="HVP batch size 列表"
    )
    parser.add_argument(
        "--sweep_tol", type=str, default="1e-3,1e-6,1e-9", help="bisection 容差列表"
    )

    # 基础参数
    parser.add_argument("--hvp_batches", type=int, default=8)
    parser.add_argument("--chunk_size", type=int, default=10)
    parser.add_argument("--num_steps", type=int, default=100, help="score 计算使用的训练 batch 数")
    parser.add_argument("--eval_batches", type=int, default=20)
    parser.add_argument("--batch_size", type=int, default=4)
    parser.add_argument("--seq_length", type=int, default=512)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--output_dir", type=str, default="results/paper_results/sensitivity")

    # 选择性运行
    parser.add_argument("--skip_k", action="store_true", help="跳过 K sweep")
    parser.add_argument("--skip_hvp", action="store_true", help="跳过 HVP batch size sweep")
    parser.add_argument("--skip_tol", action="store_true", help="跳过 bisection tolerance sweep")
    parser.add_argument("--skip_timing", action="store_true", help="跳过 timing overhead breakdown")
    ...
    if not args.skip_k:
        ...
        k_results = sweep_K(args, model_init, train_loader, val_loader, task_type, cached_eval)
    ...
    if not args.skip_hvp:
        ...
        hvp_results = sweep_hvp_batch(
            args, model_init, cached_train, cached_eval, task_type, model_family=model_family
        )
        all_results["sweep_hvp_batches"] = hvp_results
        print_results_table(hvp_results, ["hvp_batches", "hvp_time_s", "loss", "perplexity"])

    # --- Sweep bisection tolerance ---
    if not args.skip_tol:
        print("\n[3/4] Sweep bisection tolerance...")
        tol_results = sweep_bisection_tol(
            args, model_init, cached_train, cached_eval, task_type, model_family=model_family
        )
        all_results["sweep_bisection_tol"] = tol_results
        print_results_table(tol_results, ["bisection_tol", "alloc_time_s", "loss", "perplexity"])

    # --- Timing overhead breakdown ---
    if not args.skip_timing:
        print("\n[4/4] Timing overhead breakdown...")
        timing_result = timing_breakdown(
            args, model_init, cached_train, cached_eval, task_type, model_family=model_family
        )
        all_results["timing_breakdown"] = timing_result

    # 保存
    write_json(output_dir / f"{prefix}_sensitivity.json", all_results)
