"""
BERT-Large 在 SST-2 上微调的脚本。

运行命令:
    python experiments/scripts/finetune_bert_large_sst2.py \
        --num_steps 1000 \
        --batch_size 16 \
        --gradient_accumulation_steps 2 \
        --learning_rate 2e-5 \
        --save_every 200 \
        --output_dir checkpoints/bert_large_sst2_1000steps \
        --device cuda
...
from src.ckpt_compress.utils.data_loader import get_sst2_loaders
...
def train_n_steps(
    ...
    log_interval=50,
    save_interval=200,
    checkpoint_dir='./checkpoints/bert_large_sst2_1000steps',
):
    """
    训练指定步数。
...
def main():
    parser = argparse.ArgumentParser(
        description='BERT-Large 在 SST-2 上微调'
    )
    ...
    parser.add_argument('--checkpoint_dir', type=str,
                        default='./checkpoints/bert_large_sst2_1000steps',
                        help='检查点保存目录')
