...
def classify_layer(name, model_type):
    ...
    if model_type in ("gpt2", "pythia"):
        ...
        if "wte" in name_lower or "wpe" in name_lower or "embed" in name_lower:
            ...
            "c_fc" in name_lower
            or "fc1" in name_lower
            or "mlp.c_fc" in name_lower
...
def plot_violin_by_type(df, output_path, model_name):
    """按层类型绘制 violin plot。"""
    type_order = ["Embedding", "Attention-QKV", "Attention-Out", "MLP-Up", "MLP-Down", "LayerNorm"]
    ...
    try:
        ...
        sns.violinplot(
            data=df[df["layer_type"].isin(present_types)],
            x="layer_type",
            y="log_score",
            order=present_types,
            palette=palette,
...
def plot_violin_by_block(df, output_path, model_name, n_blocks_to_show=6):
    ...
    fig, axes = plt.subplots(
        1, len(selected_blocks), figsize=(4 * len(selected_blocks), 5), sharey=True
    )
    ...
    for ax, block_id in zip(axes, selected_blocks):
        ...
        try:
            ...
            sns.violinplot(
                data=block_data[block_data["layer_type"].isin(present)],
                x="layer_type",
                y="log_score",
                order=present,
                palette=palette,
            ...
            box_data = [
                block_data[block_data["layer_type"] == t]["log_score"].values for t in present
            ]
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/paper_results/layer_violin")
    ...
    plot_violin_by_type(df, output_dir / f"{args.model}_violin_by_type.png", args.model)
    plot_violin_by_block(df, output_dir / f"{args.model}_violin_by_block.png", args.model)

    # 保存数据
    stats.to_csv(output_dir / f"{args.model}_layer_stats.csv")
    write_json(output_dir / f"{args.model}_config.json", vars(args))
