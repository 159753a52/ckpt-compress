...
def _gamma_fit_summary(fit_results):
    """Return pass-count evidence, rejecting an empty successful-fit set."""
    pvalues = [
        result["gamma_pvalue"]
        for result in fit_results
        if result.get("gamma_pvalue") is not None
    ]
    if not pvalues:
        raise ValueError("Gamma pass rate requires at least one successful fit")
    return (
        sum(1 for pvalue in pvalues if pvalue > 0.05),
        len(pvalues),
        len(fit_results),
    )
    ...
    for name, score in scores_dict.items():
        ...
        for dist_name, fit_fn in distributions.items():
            try:
                ...
                row[f"{dist_name}_pvalue"] = None
                row[f"{dist_name}_aic"] = None
                row[f"{dist_name}_error"] = f"{type(exc).__name__}: {exc}"
...
def run_allocation_comparison(
    model_factory, scores, prune_ratios, cached_eval, task_type, device, baseline
):
    """对比三种分配策略的下游质量。"""
    strategies = {
    ...
    for strat_name, allocator in strategies.items():
        for ratio in prune_ratios:
            ...
            print(
                f"  {strat_name} | ratio={ratio:.0%} (actual={actual:.1%}) | "
...
def main():
    ...
    parser.add_argument(
        "--importance", type=str, default="second-order-hvp", help="用于计算 score 的方法"
    )
    ...
    parser.add_argument("--output_dir", type=str, default="results/paper_results/gamma_validation")
    ...
    train_loader, val_loader, task_type = get_data_loaders(
        args.model, args.dataset, args.batch_size, args.seq_length, data_dir=args.data_dir
    )
    ...
    # 统计 Gamma 拟合通过率
    gamma_pass, fitted_layers, total_layers = _gamma_fit_summary(fit_results)
    print(
        f"  Gamma KS pass rate (p>0.05): {gamma_pass}/{fitted_layers} successful fits "
        f"= {gamma_pass/fitted_layers*100:.1f}% "
        f"({total_layers - fitted_layers} fit failures)"
    )

    # 每种分布的平均 KS-D
    for dist in ["gamma", "lognormal", "weibull", "exponential"]:
        ks_vals = [r[f"{dist}_ks"] for r in fit_results if r.get(f"{dist}_ks") is not None]
        if ks_vals:
    ...
    alloc_results = run_allocation_comparison(
        model_factory, scores, prune_ratios, cached_eval, task_type, args.device, baseline
    )
    ...
    # 拟合结果
    write_json(
        output_dir / f"{prefix}_fit_results.json",
        {
            "schema_version": 1,
            "status": "complete",
            "config": vars(args),
    ...
    # 分配策略对比
    save_results(
        alloc_results, str(output_dir), f"gamma_validation_{args.model}_{args.dataset}", vars(args)
    )

    print("\n" + "=" * 70)
    print("分配策略对比:")
    print_results_table(
        alloc_results, ["allocation", "target_ratio", "actual_ratio", "loss", "perplexity"]
    )
    print("\n完成！")
