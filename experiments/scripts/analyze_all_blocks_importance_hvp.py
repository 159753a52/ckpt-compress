"""
使用 HVP (Hessian-Vector Product) 重要性计算方式为每个 block 生成分布图。
...
重要性公式: s_i = -g_i * θ_i + 0.5 * θ_i * (H * θ)_i
...
def compute_importance_scores_hvp_for_analysis(
    model,
    data_batches,
    device,
    num_batches=5
...
def plot_block_distributions(block_name, block_params, output_path):
    ...
    for idx, param_name in enumerate(param_names):
        ...
        ax.set_xlabel('Importance Score (HVP)', fontsize=11)
    ...
    # 设置总标题
    fig.suptitle(f'{block_name.replace("_", " ").title()} - HVP Importance Distribution',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='使用 HVP 重要性为每个 block 生成分布图')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='results/blocks_importance_hvp',
                        help='输出目录')
    ...
    print("=" * 80)
    print("GPT-2 所有 Blocks 的参数重要性分布分析（HVP 版本）")
    print("=" * 80)
    ...
    print("开始 HVP 重要性计算...")
    print("=" * 80)
    scores = compute_importance_scores_hvp_for_analysis(
        model=model,
    ...
    for block_name in blocks_to_plot:
        ...
        output_path = output_dir / f'{block_name}_importance_hvp.png'
