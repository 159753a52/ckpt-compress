"""
Pareto 曲线生成脚本 - 扫描多个稀疏率（Fig 4）

支持的方法：
- magnitude-uniform: 权重绝对值 + Uniform 分配
- first-order-uniform: 一阶梯度 + Uniform 分配
- second-order-uniform: 一阶+二阶 + Uniform 分配
- second-order-gamma: 一阶+二阶 + Gamma-adaptive 分配

运行命令:
    python experiments/scripts/comparison/run_pareto.py \
        --checkpoint checkpoints/gpt2_small_wikitext103_1000steps/checkpoint_step_1000_final.pt \
        --model gpt2-small \
        --methods magnitude-uniform,first-order-uniform,second-order-uniform,second-order-gamma \
        --min_ratio 0.05 \
        --max_ratio 0.90 \
        --step 0.05 \
        --num_steps 100 \
...
def plot_pareto_curve(df, output_path):
    """绘制 Pareto 曲线"""
    ...
    for i, method in enumerate(methods):
        method_data = df[df['method'] == method].sort_values('prune_ratio')
        plt.plot(
            method_data['prune_ratio'] * 100,
    ...
    plt.title('Pareto Curve: Pruning Ratio vs Perplexity', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✓ Pareto 曲线已保存到: {output_path}")
    plt.close()


def run_pareto_experiment(args):
    """运行 Pareto 曲线实验"""
    ...
    if any('magnitude' in m for m in methods):
        all_scores['magnitude'] = filter_prunable_params(
            compute_importance_scores_magnitude(weights)
        )
    
    if any('first-order' in m for m in methods):
        all_scores['first-order'] = filter_prunable_params(
            compute_importance_scores_first_order(weights, gradients, exp_avg_sq, args.alpha)
        )
    
    if any('second-order' in m for m in methods):
        all_scores['second-order'] = filter_prunable_params(
            compute_importance_scores_abs(weights, gradients, exp_avg_sq, args.alpha)
        )
    ...
    # 5. 扫描稀疏率
    print("\n[5/6] 扫描稀疏率...")
    prune_ratios = np.arange(args.min_ratio, args.max_ratio + args.step, args.step)
    ...
    for method in methods:
        ...
        # 解析方法名
        parts = method.split('-')
        importance_type = parts[0]  # magnitude, first-order, second-order
        allocation_type = parts[1] if len(parts) > 1 else 'uniform'  # uniform, gamma
        
        scores = all_scores[importance_type]
        
        for prune_ratio in tqdm(prune_ratios, desc=f"  {method}"):
            # 剪枝
            if allocation_type == 'uniform':
                masks, actual_ratio = uniform_pruning(scores, prune_ratio)
            elif allocation_type == 'gamma':
                masks, actual_ratio = gamma_adaptive_pruning(scores, prune_ratio)
            else:
                raise ValueError(f"未知的分配类型: {allocation_type}")
            
            # 评估
            loss = apply_masks_and_evaluate(model, masks, cached_eval_batches, device)
    ...
    csv_path = output_dir / f'pareto_{args.model}_{timestamp}.csv'
    png_path = output_dir / f'pareto_curve_{args.model}_{timestamp}.png'
...
def main():
    parser = argparse.ArgumentParser(description='Pareto 曲线生成 - 扫描多个稀疏率')
    parser.add_argument('--checkpoint', type=str, required=True, help='模型检查点路径')
    parser.add_argument('--model', type=str, default='gpt2-small', choices=['gpt2-small', 'gpt2-medium'], help='模型类型')
    parser.add_argument('--methods', type=str, 
                        default='magnitude-uniform,first-order-uniform,second-order-uniform,second-order-gamma',
                        help='方法列表（逗号分隔）')
    parser.add_argument('--min_ratio', type=float, default=0.05, help='最小剪枝率')
    parser.add_argument('--max_ratio', type=float, default=0.90, help='最大剪枝率')
    parser.add_argument('--step', type=float, default=0.05, help='剪枝率步长')
    ...
    parser.add_argument('--output_dir', type=str, default='results/pareto', help='输出目录')
    
    args = parser.parse_args()
    run_pareto_experiment(args)
