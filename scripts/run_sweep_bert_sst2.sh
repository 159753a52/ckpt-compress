#!/bin/bash
# 轻量多稀疏率 sweep：BERT-Large / SST-2
# K=2, 200步, 4个稀疏率
# 预计总耗时 ~3-4 小时（每个稀疏率 ~50 分钟）
...
echo "=== BERT-Large SST-2 Multi-Ratio Sweep ==="
...
for RATIO in 0.1 0.2 0.3 0.4; do
    ...
    python -u experiments/scripts/run_fault_tolerant_training.py \
        ...
        --eval_interval 10 \
        --device cuda \
        --output_dir "results/paper_results/sweep_bert_sst2_K2_p${RATIO}"
done
