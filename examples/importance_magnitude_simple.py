...
def example_3_simulated_model():
    """示例 3: 在模拟模型上使用"""
    print("\n" + "=" * 60)
    print("示例 3: 在模拟神经网络上计算权重绝对值重要性")
    print("=" * 60)

    # 模拟一个小型神经网络的权重
    torch.manual_seed(42)
    weights = {
        'embedding.weight': torch.randn(1000, 128) * 0.1,
        'layer1.weight': torch.randn(128, 256) * 0.05,
        'layer1.bias': torch.randn(256) * 0.01,
        'layer2.weight': torch.randn(256, 256) * 0.05,
        'layer2.bias': torch.randn(256) * 0.01,
        'layer3.weight': torch.randn(256, 128) * 0.05,
        'layer3.bias': torch.randn(128) * 0.01,
        'output.weight': torch.randn(128, 10) * 0.1,
        'output.bias': torch.randn(10) * 0.01,
    }

    print(f"模拟神经网络，共 {len(weights)} 个参数层")
